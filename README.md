...
### Quick Start
To run the entire server setup with a single script:
...
The `start_project.sh` script (included in this repository) will:
- Check prerequisites (virtual environment, required commands)
- Start Redis server
- Start Django development server
- Start 4 Celery workers for load balancing
- Start Celery Beat scheduler
- Provide status updates and process IDs
- Handle cleanup on exit

**Full script content** (`start_project.sh`):
